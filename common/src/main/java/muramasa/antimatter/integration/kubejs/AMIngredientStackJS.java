...
public class AMIngredientStackJS extends IngredientStackJS {
    protected boolean nonConsume = false;
    protected boolean ignoreNbt = false;

    public AMIngredientStackJS(IngredientJS i, int c) {
        super(i, c);
    }

    public AMIngredientStackJS setNoConsume(boolean noConsume) {
    ...
    public AMIngredientStackJS setIgnoreNbt(boolean ignoreNbt) {
    ...
    public static AMIngredientStackJS fromJson(@Nullable JsonElement json) {
        ...
        return new AMIngredientStackJS(IngredientJS.of(js.size() == 1 ? js.get(0) : js), count).setIgnoreNbt(ignoreNBt).setNoConsume(noConsume);
    }
