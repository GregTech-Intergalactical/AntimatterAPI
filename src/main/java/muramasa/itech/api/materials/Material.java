...
//    public static Materials Paper = new Materials(879, "Paper", 0xfafafa, PAPER).asDust(); //TODO needed?
    ...
    public static Material Redstone = new Material(810, "Redstone", 0xc80000, ROUGH).asDust().addOre().add(Silicon, 1, Pyrite, 5, Material.Ruby, 1, Mercury, 3);
    public static Material Basalt = new Material(844, "Basalt", 0x1e1414, ROUGH).asDust().add(Material.Olivine, 1, Calcite, 3, Flint, 8, DarkAsh, 4);
...
//        }
    ...
    private Material(int id, String name, int rgb, MaterialSet set, Element element) {
    ...
    private Material asSolid(int... temps) {
        asDust(temps);
        add(INGOT);
    ...
    private Material asMetal(int... temps) {
        asSolid(temps);
        add(METAL);
        return this;
    }
    ...
    private Material asFluid(int... fuelPower) {
        add(FLUID);
        if (fuelPower.length >= 1) this.fuelPower = fuelPower[0];
        return this;
    }

    private Material asGas(int... fuelPower) {
        asFluid(fuelPower);
        add(GAS);
        return this;
    }

    private Material asPlasma(int... fuelPower) {
        asFluid(fuelPower);
        add(GAS, PLASMA);
        return this;
    }

    private Material addOre() {
        add(CRUSHED, CRUSHEDC, CRUSHEDP);
        return this;
    }

    private Material addTools(float toolSpeed, int toolDurability, int toolQuality) {
        if (hasFlag(INGOT)) {
