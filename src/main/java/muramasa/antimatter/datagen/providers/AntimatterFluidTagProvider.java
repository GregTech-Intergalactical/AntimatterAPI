...
public class AntimatterFluidTagProvider extends ForgeFluidTagsProvider implements IAntimatterProvider {
    ...
    public void run() {
        ...
        registerTags();
        Utils.getTags(Item.class).forEach(t -> DynamicResourcePack.addTag("fluids", t.getName(), getOrCreateBuilder(t).getInternalBuilder().serialize()));
    ...
    protected void processTags(String domain) {
        AntimatterAPI.all(AntimatterFluid.class).forEach(f ->
                getOrCreateBuilder(Utils.getForgeFluidTag(f.getId()))
