...
public class RubberTree extends Tree {
    ...
    public RubberTree(){
        trunkBlocks.func_227407_a_(Data.RUBBER_LOG.getDefaultState().with(BlockRubberLog.RESIN_STATE, BlockRubberLog.ResinState.FILLED).
        ...
        trunkBlocks.func_227407_a_(Data.RUBBER_LOG.getDefaultState().with(BlockRubberLog.RESIN_STATE, BlockRubberLog.ResinState.NONE), 10);
    }
    @Nullable
    @Override
    protected ConfiguredFeature<TreeFeatureConfig, ?> func_225546_b_(Random rand, boolean flowers) {
        return Feature.NORMAL_TREE.withConfiguration(Data.RUBBER_TREE_CONFIG_BLOB);
    }
    @Override
    public boolean func_225545_a_(IWorld world, ChunkGenerator<?> chunkGenerator, BlockPos pos, BlockState state, Random random) {
        ConfiguredFeature<TreeFeatureConfig, ?> configuredfeature = Feature.NORMAL_TREE.withConfiguration(
                world.getBiome(pos).getDefaultTemperature() <= 0.25 ? Data.RUBBER_TREE_CONFIG_SPRUCE : Data.RUBBER_TREE_CONFIG_BLOB);
