...
public class BakedDynamicOld extends BakedBase {

    private BlockDynamicOld block;

    public BakedDynamicOld(BlockDynamicOld block) {
    ...
    public IModelData getModelData(@Nonnull IEnviromentBlockReader world, @Nonnull BlockPos pos, @Nonnull BlockState state, @Nonnull IModelData data) {
        if (((BlockDynamicOld) state.getBlock()).getLookup().size() == 0) {
            data.setData(AntimatterProperties.DYNAMIC_CONFIG, new int[1]);
        } else {
            data.setData(AntimatterProperties.DYNAMIC_CONFIG, ((BlockDynamicOld) state.getBlock()).getConfig(state, world, new BlockPos.MutableBlockPos(pos), pos));
        }
        return data;
    }
