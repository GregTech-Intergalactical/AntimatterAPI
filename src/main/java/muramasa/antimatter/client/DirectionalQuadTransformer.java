...
public class DirectionalQuadTransformer
{
    ...
    public DirectionalQuadTransformer(TransformationMatrix transform)
    {
     ...
     */
    public BakedQuad processOne(BakedQuad input, Direction newSide)
    {
        int[] inData = input.getVertices();
        int[] outData = Arrays.copyOf(inData, inData.length);
        processVertices(inData, outData);

        return new BakedQuad(outData, input.getTintIndex(), newSide, input.getSprite(), input.isShade());
    }
     ...
     */
    public List<BakedQuad> processMany(List<BakedQuad> inputs, Direction newSide)
    {
        if(inputs.size() == 0)
        ...
        for(BakedQuad input : inputs)
        {
            ...
            outputs.add(new BakedQuad(outData, input.getTintIndex(), newSide, input.getSprite(), input.isShade()));
        }
        return outputs;
    }
