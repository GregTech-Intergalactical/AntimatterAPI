...
public class FeatureVein extends AntimatterFeature<NoneFeatureConfiguration> {
    ...
    private static final HashMap<ChunkPos, Integer> heightCache = new HashMap<ChunkPos, Integer>();
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> placer) {
        ...
        // calculate how man chunks we need to search for in each direction to find vane chunks based on the largest
        ...
        for (int layer: WorldGenVein.getAllLayers()) {
            ...
            final int layerChunks = 1 + Math.max(1, (int) Math.ceil(((double) WorldGenVein.getMaxLayerSize(layer) - 16) / 32 * 1.25));
            ...
            for (int xOffset = -layerChunks; xOffset <= layerChunks; xOffset++) {
                for (int zOffset = -layerChunks; zOffset <= layerChunks; zOffset++) {
                    ...
                    if (random.nextFloat() <= WorldGenVein.getLayerChance(layer)) {
                        int chunkMaxY = Math.min(worldMaxY, getChunkMaxHeight(placer, veinChunkX, veinChunkZ, veinCenterX, veinCenterZ));
                        // place vane in the current chunk
                        if (this.placeVane(random, world, worldMinY, chunkMaxY, chunkCornerX, chunkCornerZ, veinCenterX, veinCenterZ, worldMinY, worldMaxY, layerVeins)) {
                            placed = true;
                        }
                    }
                }
    ...
    private boolean placeVane(Random random, WorldGenLevel world, int chunkMinY, int chunkMaxY, int chunkCornerX, int chunkCornerZ, int veinCenterX, int veinCenterZ, int worldMinY, int worldMaxY, List<WorldGenVein> veins) {
        final List<WorldGenVein> validVeins = veins.stream().filter(vein -> (vein.minY >= chunkMinY && vein.minY <= chunkMaxY) || (vein.maxY >= chunkMinY && vein.maxY <= chunkMaxY)).collect(Collectors.toList());
        if (validVeins.size() == 0) {
            return false;
        }
        final WorldGenVein vein = validVeins.get(random.nextInt(validVeins.size()));
        final @Nullable WorldGenVeinVariant variant = vein.variants.size() > 0 ? vein.variants.get(random.nextInt(vein.variants.size())) : null;
        ...
        // check all positions in the current chunk (with limited y) if they are part of the vane
        for (int x = chunkCornerX; x < chunkCornerX + 16; x++) {
            for (int z = chunkCornerZ; z < chunkCornerZ + 16; z++) {
                for (int y = minCheckY; y < maxCheckY; y++) {
                    if (isInEllipse(random, veinCenterX, veinCenterY, veinCenterZ, x, y, z, veinSizeX / 2, veinSizeY / 2, veinSizeZ / 2)) {
                        ...
                        if (variant != null && variant.materials.size() > 0) {
                            ...
                            final boolean spawnSmallOre = !spawnOre && variant.smallOreChance != 0 && currentOreChance <= variant.oreChance + variant.smallOreChance;
                            if (spawnOre || spawnSmallOre) {
                                int currentY = y;
                                final List<WorldGenVeinVariantMaterial> validMaterials = variant.materials.stream().filter(material -> currentY >= material.minY && currentY <= material.maxY).collect(Collectors.toList());
                                if (validMaterials.size() > 0) {
                                    final WorldGenVeinVariantMaterial material = validMaterials.get(random.nextInt(validMaterials.size()));
                                    WorldGenHelper.setOre(world, pos, material.material, spawnSmallOre ? Data.ORE_SMALL : Data.ORE);
                                    placed = true;
                                }
                            }
                        }
        ...
        if (placed && variant != null && variant.surfaceStoneChance > 0) {
            this.placeSurfaceStones(random, world, variant.surfaceStoneChance, chunkCornerX, chunkCornerZ, variant.materials, vein.fill);
        }
        if (placed && variant != null && variant.markerOreChance > 0) {
            this.placeOreMarkers(random, world, variant.markerOreChance, chunkCornerX, chunkCornerZ, variant.materials, worldMinY, worldMaxY);
        }

        return placed;
    }

    private void placeSurfaceStones(Random random, WorldGenLevel world, float chance, int chunkCornerX, int chunkCornerZ, List<WorldGenVeinVariantMaterial> materials, @Nullable() BlockState fill) {
        for (int x = chunkCornerX; x < chunkCornerX + 16; x++) {
            for (int z = chunkCornerZ; z < chunkCornerZ + 16; z++) {
                if (random.nextFloat() <= chance) {
                    int y = Math.min(world.getHeight(Heightmap.Types.OCEAN_FLOOR, x, z), world.getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, x, z));
                    final BlockPos pos = new BlockPos(x, y, z);
    ...
    private void placeOreMarkers(Random random, WorldGenLevel world, float chance, int chunkCornerX, int chunkCornerZ, List<WorldGenVeinVariantMaterial> materials, int worldMinY, int worldMaxY) {
        for (int x = chunkCornerX; x < chunkCornerX + 16; x++) {
            for (int z = chunkCornerZ; z < chunkCornerZ + 16; z++) {
    ...
    private static boolean isInEllipse(Random random, int centerX, int centerY, int centerZ, int x, int y, int z, int sizeX, int sizeY, int sizeZ) {
        double diff = (
            (Math.pow((x - centerX), 2) / Math.pow(sizeX, 2)) +
    ...
    private static int getChunkMaxHeight(FeaturePlaceContext<NoneFeatureConfiguration> placer, int veinChunkX, int veinChunkZ, int veinCenterX, int veinCenterZ) {
        ...
        if (existing != null) {
            ...
            final int height = placer.chunkGenerator().getBaseHeight(veinCenterX, veinCenterZ, Heightmap.Types.OCEAN_FLOOR_WG, chunk.getHeightAccessorForGeneration());
