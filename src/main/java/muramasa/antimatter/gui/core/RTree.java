...
public class RTree<T extends IGuiElement> {
    ...
    public void insert(T w) {
        ...
        insert(new float[]{(float) x1, (float) y1}, new float[]{(float) width, (float) height}, w);
    }
    ...
    public boolean delete(T w) {
        ...
        int width = w.getW();
        int height = w.getH();
        return delete(new float[]{(float) x1, (float) y1}, new float[]{(float) width, (float) height}, w);
    }
    ...
    public interface NodeVisitor<V> {
        void visit(int depth, float[] coords, float[] dimensions, V value);
    }
