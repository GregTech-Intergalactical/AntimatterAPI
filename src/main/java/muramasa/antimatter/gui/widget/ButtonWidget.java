...
public class ButtonWidget extends Widget {
    ...
    protected ButtonWidget(GuiInstance instance, boolean isRemote, ResourceLocation res, @Nullable ButtonBody body, @Nullable ButtonOverlay overlay, Consumer<ButtonWidget> onPress) {
        super(instance);
        this.res = res;
        this.body = body;
    ...
    protected void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (body != null) {
            ...
            ScreenWidget.blit(matrixStack, x, y, this.getW(), this.getH(), xTex, yTex, body.getW(), body.getH(), 256, 256);
        }
        ...
        if (overlay != null) {
       //     ScreenWidget.blit(matrixStack, screen().getGuiLeft() + x, screen.getGuiTop() + y, width, height, overlay.getX(), overlay.getY(), overlay.getW(), overlay.getH(), 256, 256);
        } else if (this.bodyLoc != null) {
            minecraft.getTextureManager().bindTexture(this.bodyLoc);
      //      ScreenWidget.blit(matrixStack, screen().getGuiLeft() + x, screen.getGuiTop() + y, width, height, resLoc.x, resLoc.y, resLoc.z, resLoc.w, 256, 256);
        }
    ...
    protected ButtonWidget(GuiInstance instance, IGuiElement parent, boolean isRemote, ResourceLocation res, @Nullable ResourceLocation bodyLoc, @Nullable int4 loc, @Nullable ButtonOverlay overlay, Consumer<ButtonWidget> onPress) {
        super(instance);
        this.res = res;
        this.body = null;
        this.overlay = overlay;
